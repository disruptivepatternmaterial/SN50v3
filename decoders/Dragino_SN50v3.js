 ...
 * for stock SHT20/SHT31 units — the SHT45 uses the same payload slots.
 *
 * Deployed devices: la666152750 (Forest Lands High Point, SHT2x) and the
 * SHT45 units (e.g. la666152752, DevEUI A84041D1315DE230).
 *
 * Source of truth: this repo's firmware,
 *   Projects/Applications/DRAGINO-LRWAN-AT/src/main.c (payload packing).
 *
 * Ports:
 *   fPort 2  — sensor uplink, workmode 1 (IIC), 11 bytes:
 *              Bat(2) | DS18B20(2) | ADC(2) | flags(1) | SHT_Temp(2) | SHT_Hum(2)
 *              flags byte (bytes[6]): bit7 = PA8 level, bit1 = IN1 level,
 *              bit0 = interrupt flag; bits 6:2 = mode (0 in workmode 1).
 *              If a BH1750 is fitted instead of an SHT, bytes[7..8] carry
 *              illuminance and bytes[9..10] are 0x0000.
